...
async function containerSetup(): Promise<void> {
  ...
  if (manifest.cliTarball) {
    ...
    bakeStep = "COPY cli.tgz /tmp/cli.tgz\nRUN npm install -g --no-audit --no-fund /tmp/cli.tgz && n8n-decanter --version";
  } else {
    bakeStep = `RUN npm install -g --no-audit --no-fund ${manifest.decanterSpec} && n8n-decanter --version`;
  }
