...
| `scenarios/S1–S17.md` + `STYLE.md` | Persona / goal / adaptive-beats / checklist + a machine-readable `## Orchestration` turn spine; blinding rules verbatim. Structurally checked offline by `test/unit/field-scenarios.test.mts` (part of `npm test`). |
...
| Agent wiring when the sync dir is nested in a bigger repo | **S16** (`FIELD_NESTED=1`) | **runnable** — the layout behind [Plan 81](../../plans/done/81-nested-syncdir-agent-wiring.md); 3 valid rounds, all PASS. `ftrun-468939` found the agent misdiagnosing the missing tools as "restart needed" ([Plan 82](../../plans/done/82-nested-field-test-condition.md)); `ftrun-526356` re-measured after the wording fix and got both causes back ([Plan 83](../../plans/done/83-missing-tools-nested-diagnosis.md)); `ftrun-114072` ran on the [Plan 86](../../plans/done/86-init-writes-when-asked-for-help.md) build as a pre-release check and **never reached for `init` at all** — it shipped the edit from the repo root and named the missing MCP wiring in its own words. All three agents took Option A unprompted — `--dir` still unexercised by a blind round, and so is Plan 86's nested-`init` refusal: the round says agents do not take the path that burned the user who reported it, which is a finding about agents, not a test of the guard |
...
| `FIELD_WORKTREE=1` | **linked-worktree condition**: the scaffolded project is committed and a `git worktree add` sibling becomes the session's world, so every gitignored file (credentials, `node_modules`) exists only in the main checkout. Manifest gains `worktree` + `mainCheckout`; `workDir` names the worktree. The CLI stays on PATH via the staged global prefix, so this measures the worktree and not S6's install question — combine with `FIELD_NO_PATH_HELP=1` for the harder variant. Required by S17, host-mode only (a fenced container mounts the sync dir alone, so the worktree's `.git` pointer would resolve to nothing). |
