...
**Model:** **Sonnet** — well-scoped, API-shaped work (thin verbs, a couple of
threaded response fields, one warning) gated on smoke verification, no novel
design; the semantics are already nailed in `PLAN.md`. Give the destructive
`delete` path (confirmation gate, irreversibility) a careful review pass —
Opus if you want a second set of eyes on that one verb.
