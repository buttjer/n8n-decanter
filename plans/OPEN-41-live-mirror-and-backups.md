# Plan 41 — Live snapshot mirror + git-native workflow backups
...
- **Plan 0 / BACKLOG.md:** "*Auto-refresh `workflow.json` after structure edits
  over MCP*" (2026-07-23) → **Part A**. Its open questions (which ops, debounce/
  races, churn, draft-vs-tip, on-by-default vs opt-in) are resolved below.
...
- CI green; docs three-surfaces in sync; the backlog item checked off; PLAN.md
  updated (below).
...
- **BACKLOG:** the auto-refresh item → graduated (Part A). The backup/DR store is
  a **distinctive feature** (git-native versioned disaster recovery — neither n8n
  nor generic git-sync offers it) → its own distinctive-features entry.
