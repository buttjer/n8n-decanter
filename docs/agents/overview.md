...
| `test` | Executes the workflow's **draft** on the instance (pinned trigger/network nodes, real logic nodes) — code runs remotely, but the live version is never affected and non-interactive runs never write. Because it runs the **draft**, it is only meaningful **after a `push`** — before one, the draft holds the old code (or nothing). It is the post-push runtime check in `preflight → push → test → publish`. |
