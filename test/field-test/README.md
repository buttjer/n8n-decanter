...
| **n8n MCP tools going missing in a linked git worktree** | **S17** (`FIELD_WORKTREE=1`) | **runnable** — 1 round (`ftrun-71032`, PASS / MCP-MISSING-RECOVERED). Reproduced the reported symptom: turn 1 opened with `n8n-instance` `"failed"` and **0** of its tools, because `npx --no-install` cannot resolve without `node_modules`. The agent fixed the install in one hop and shipped through files + `push` — but **the tools only returned in the next process**, which is the finding: repairing the install does not restore MCP in-session and nothing says a restart is needed. `MCP-WORKING` needs a stage where the guard can start (global install or `symlinkDirectories`); `COPIED-CREDENTIALS` needs a build without the #287 fallback |
