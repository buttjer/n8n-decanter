...
32. [MCP-native code layer](DONE-32-mcp-native-code-layer.md) — strategy shift:
...
36. [`preflight` verb — the verification ladder as one scored gate](OPEN-36-preflight-verb.md) —
    ...
    parity, snapshot, executions health, capture freshness), then pinned
    runtime (`test`, `--full` adds `simulate`) — and condenses them into a
...
38. [README overhaul: shop window](DONE-38-readme-slim.md) — cut the 473-line
    ...
    audit found every fact already covered there — no porting needed); verb
