...
await step("watch: debounce coalesces, queued save re-pushes, close() stops", async () => {
  ...
  const config = {
    ...
    browserReload: "off" as const, proxyPort: 0, requestTimeoutMs: 30_000, dataTables: true, liveMirror: true, backupLimit: 20, host: env.N8N_HOST!, apiKey: "test-key",
  };
  const logs: string[] = [];
...
await step("watch<->proxy: single-node push broadcasts a 'pushed' SSE event through the dev-reload proxy", async () => {
  ...
  const config = {
    configDir: TMP, root: ROOT, workflows: ["wf123"], commitOnPush: false, commitOnPull: false,
    browserReload: "proxy" as const, proxyPort: 0, requestTimeoutMs: 30_000, dataTables: true, liveMirror: true, backupLimit: 20, host: env.N8N_HOST!, apiKey: "test-key",
  };
