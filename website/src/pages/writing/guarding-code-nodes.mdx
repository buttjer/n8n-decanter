...
Watching it happen is more convincing than describing it. The structural write
sails through; the source write hits the one closed door and comes back with an
address:
...
1. **Compliance.** Layout and structural violations are hard errors — including
   a `$('Some node')` call naming a node the workflow doesn't have. `--force`
...
Here is the whole shape of it: a structural edit that looked fine going in, a
publish that isn't allowed to be, and production serving the old version the
entire time.
...
moving a step, fixing the error path — through the engine's own validated API.
