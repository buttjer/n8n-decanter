...
- Depends on [Plan 3](DONE-3-local-run-and-diff-fidelity.md) task C (execution
  datasets pulled to `workflows/<Name>/executions/<execId>.json`) as the
  fixture source — **satisfied 2026-07-19: the `executions` verb ships it**
  (note: the dirs are gitignored temp data; this plan's fixture consumption
  may need a keep/pin story).
